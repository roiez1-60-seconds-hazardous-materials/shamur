...
export default function ProfilePage({ onNavigate, onClose }: {
  ...
  const menuItems = [
    ...
    { icon: '📚', label: 'ספר שנתי', sub: `${stats.totalEntries} ערכים השנה · PDF`, page: 'yearbook' },
    { icon: '⚙️', label: 'הגדרות', sub: 'התראות · תמה · נתונים', page: 'settings' },
  ];
  ...
  return (
    <div className="flex flex-col h-full bg-shamur-paper">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide">
        ...
        {/* Stats */}
        <div className="flex mx-4 mb-4 rounded-2xl overflow-hidden border border-shamur-gold/20">
          {[
        ...
        <div className="mx-4 space-y-2 pb-8">
          {menuItems.map(item => (
            <button key={item.page} onClick={() => onNavigate(item.page)}
              className="w-full flex items-center gap-4 bg-shamur-cream rounded-2xl px-4 py-4 hover:bg-shamur-gold/10 transition-colors active:scale-98">
